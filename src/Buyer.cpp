...
Buyer::create(const std::string& id, const std::string& name, const std::string& email,
              const std::string& password, double balance) {
    // check and add id
    ...
    return std::unique_ptr<Buyer>(new Buyer(id, name, email, password, balance));
}
...
bool Buyer::hasEnoughBalance(double price) {
...
std::expected<void, std::string> Buyer::addToCart(const std::shared_ptr<Product>& p, int qty) {
...
std::expected<void, std::string> Buyer::reduceCartQuantity(const std::string& productId, int qty) {
    return this->_cart.reduceQuantity(productId, qty);
}

void Buyer::viewCart() const {
    this->_cart.displayCart();
}
