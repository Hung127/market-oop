...
bool MarketDTO::addProduct(const std::shared_ptr<ProductDTO>& product) {
    if (!product) {
        return false;
    }
    ...
    if (it != _listedProducts.end()) {
        ...
        it->second = std::weak_ptr<ProductDTO>(product);
        return true;
    }

    std::weak_ptr<ProductDTO> p(product);
...
std::shared_ptr<ProductDTO> MarketDTO::findProduct(const std::string& id) const {
    auto p = this->_listedProducts.find(id);
    if ((p != this->_listedProducts.end())) {
        if (std::shared_ptr<ProductDTO> product = p->second.lock()) {
            return product;
        }
    }
    return nullptr;
...
std::vector<std::shared_ptr<ProductDTO>> MarketDTO::getAllProducts() const {
...
int MarketDTO::countAllProducts() const {
    ...
    for (auto it = this->_listedProducts.begin(); it != this->_listedProducts.end();  // NOLINT
         ++it) {
        if (std::shared_ptr<ProductDTO> p = it->second.lock()) {
            ++result;
        }
    }
    return result;
